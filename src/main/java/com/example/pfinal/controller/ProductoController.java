...
public class ProductoController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> updateCarrera(@PathVariable("id") Long id, @Valid @RequestBody Producto producto){
		Optional<Producto> carData = productoService.read(id);
	      if (carData.isPresent()) {
	        Producto dbcat = carData.get();
	        dbcat.setNombre(producto.getNombre());
	        dbcat.setPrecio(producto.getPrecio());
	        dbcat.setCantidad(producto.getCantidad());
	        dbcat.setEstado(producto.getEstado());
	        dbcat.setId(producto.getId());
	        return new ResponseEntity<Producto>(productoService.update(dbcat), HttpStatus.OK);
	      } else {
	        return new ResponseEntity<>(HttpStatus.NOT_FOUND);
	      }
}
